...
{
	uint val = 0;
	uint year_type = jdate_is_leapyear(date);
...
uint jdate_leapyears(const struct JDate *date)
{
	...
	return num + (jdate_is_past_leapday(date) || jdate_is_leapday(date));
}
...
void jdate_from_days(struct JDate *date, unsigned long days)
{
	...
	s_uint month = 0;
	uint year, rem, leap;
	static const int years_400 = (365 * 4 + 1) * 100 - 3;
	static const int years_100 = (365 * 4 + 1) * 25 - 1;
	static const int years_4 = (365 * 4 + 1);

	year = (days / years_400) * 400;
	year += ((days % years_400) / years_100) * 100;
	year += (((days % years_400) % years_100) / years_4) * 4;
	rem = (((days % years_400) % years_100) / years_4) % 4;

	struct JDate temp = {1, 1, year};
	leap = jdate_is_leapyear(&temp);
